...
class LlmRecognitionRepository @Inject constructor(
    ...
    private suspend fun recognizeWithFallback(
        ...
        val fallback = when (preferred) {
            LlmProvider.GEMINI -> LlmProvider.OPENAI
            LlmProvider.OPENAI -> LlmProvider.GEMINI
        }
        val primaryResult = invokeProvider(preferred, buildPrompt)
        if (primaryResult.isSuccess) {
            return primaryResult
        }
        if (apiKeyStore.hasLlmProvider(fallback)) {
            return invokeProvider(fallback, buildPrompt)
        }
        return primaryResult
    }
