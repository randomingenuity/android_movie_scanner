...
- **Set Location** (map-pin icon beside Done when no location is set) opens a prompt to name the shelf or bin for this batch; once saved, the location name appears as a clickable link in place of the icon and pre-fills the Location field on each review form during bulk processing.
